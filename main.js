...
for(let i=0; i<TempCart.length; i++){
    ...
    <input type="number" id="numberProductCart${i}" value= '${TempCart[i].prices}' onchange="changeQtyCart(${i})">
...
function changeQtyCart(index){
    ...
    // console.log(TempCart[index].prices);
    sumProduct();
    sumPrices();
}
function sumProduct(){
    let sum =0;
    for(let i=0; i<TempCart.length; i++){
        sum += +(TempCart[i].prices);
    }
    document.getElementById('qtyTotalShow').innerText = sum;
}
function sumPrices(){
    let sum = 0;
    for(let i =0; i< TempCart.length; i++){
        sum += (+(TempCart[i].qty))* (+(TempCart[i].prices))
    }
    document.getElementById('pricesTotalShow').innerText = sum + ' đồng'
}
renderCart();